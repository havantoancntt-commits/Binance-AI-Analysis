...
const Ticker: React.FC<TickerProps> = ({ coinPair, tickerData }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:flex-wrap justify-between items-start sm:items-center gap-x-4 gap-y-1 w-full">
      <div className="flex items-center gap-3">
        <h2 className="text-lg sm:text-xl font-bold text-white">{coinPair}</h2>
      </div>
      
      {tickerData ? (
        <div className="flex items-center gap-4 sm:gap-6">
            <div className={`flex items-baseline p-1 rounded-md transition-colors duration-700 ${priceAnimation}`}>
                <span className="text-xl sm:text-2xl font-bold text-white">${tickerData.price}</span>
            </div>
