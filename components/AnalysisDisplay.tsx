...
const RecommendationCard: React.FC<{ recommendation: Recommendation }> = ({ recommendation }) => {
    ...
    switch (recommendation.signal) {
        case 'Strong Buy': icon = <RocketLaunchIcon />; text = t('analysis.recommendation.strongBuy'); mainColor='text-red-400'; gradientFrom='from-red-500/20'; gradientTo='to-red-500/0'; borderColor='border-red-500/50'; shadowColor='shadow-red-500/20'; break;
        case 'Buy': icon = <ArrowTrendingUpIcon />; text = t('analysis.recommendation.buy'); mainColor='text-orange-400'; gradientFrom='from-orange-500/20'; gradientTo='to-orange-500/0'; borderColor='border-orange-500/50'; shadowColor='shadow-orange-500/20'; break;
        ...
        case 'Sell': icon = <ArrowTrendingDownIcon />; text = t('analysis.recommendation.sell'); mainColor='text-purple-400'; gradientFrom='from-purple-500/20'; gradientTo='to-purple-500/0'; borderColor='border-purple-500/50'; shadowColor='shadow-purple-500/20'; break;
...
const SentimentIndicator: React.FC<{ sentiment: AnalysisResult['marketSentiment'] }> = ({ sentiment }) => {
    const { t } = useTranslation();
    const sentimentConfig = { 'Extreme Fear': { text: t('analysis.sentiment.extremeFear'), color: 'text-fuchsia-500', value: 10 }, 'Fear': { text: t('analysis.sentiment.fear'), color: 'text-purple-400', value: 30 }, 'Neutral': { text: t('analysis.sentiment.neutral'), color: 'text-orange-400', value: 50 }, 'Greed': { text: t('analysis.sentiment.greed'), color: 'text-amber-400', value: 70 }, 'Extreme Greed': { text: t('analysis.sentiment.extremeGreed'), color: 'text-yellow-300', value: 90 } };
...
const MultiTimeframeTrend: React.FC<{ trendAnalysis: AnalysisResult['trendAnalysis'] }> = ({ trendAnalysis }) => {
    ...
    return (
        <StatCard title={t('analysis.card.title.trend')} icon={<ChartBarSquareIcon className="w-5 h-5" />}><div className="flex justify-between items-center divide-x divide-gray-700"><TrendItem title={t('analysis.trend.short')} trendInfo={trendAnalysis.shortTerm} /><TrendItem title={t('analysis.trend.medium')} trendInfo={trendAnalysis.mediumTerm} /><TrendItem title={t('analysis.trend.long')} trendInfo={trendAnalysis.longTerm} /></div></StatCard>
    );
};
            ...
            <StatCard title={t('analysis.card.title.tradeZone')} icon={<TableCellsIcon className="w-5 h-5"/>}>
                <div className="divide-y divide-gray-800/70">
                    ...
                    {analysis.takeProfitLevels.map((level, i) => (
                        <LevelItem key={`tp-${i}`} label={t('analysis.levels.takeProfit', { index: i + 1 })} value={formatPrice(level)} color="text-yellow-300" />
                    ))}
                    <LevelItem label={t('analysis.levels.stopLoss')} value={formatPrice(analysis.stopLoss)} color="text-red-400 font-bold" />
                </div>
            </StatCard>
                ...
                <h3 style={styles.h3}>{t('pdf.section.tradeSetup')}</h3>
                 <div style={styles.grid}>
                    <table style={{...styles.table, marginTop: 0}}><tbody>
                        ...
                        <tr><td style={{...styles.tdLabel, color: '#dc2626'}}>{t('pdf.stopLoss')}</td><td style={{...styles.td, fontWeight: 'bold'}}>{formatPrice(analysis.stopLoss)}</td></tr>
                    </tbody></table>
                    <table style={{...styles.table, marginTop: 0}}><tbody>
      ...
      <div className="flex-grow flex flex-col">
        ...
        <div className="border-b border-gray-700/50 px-2 sm:px-4">
          <nav className="-mb-px flex flex-wrap sm:flex-nowrap" aria-label="Tabs">
            <TabButton label={t('analysis.tabs.overview')} icon={<InformationCircleIcon className="w-5 h-5" />} isActive={activeTab === 'overview'} onClick={() => setActiveTab('overview')} activeColor="border-orange-500" />
            <TabButton label={t('analysis.tabs.setup')} icon={<TableCellsIcon className="w-5 h-5" />} isActive={activeTab === 'setup'} onClick={() => setActiveTab('setup')} activeColor="border-cyan-500" />
            <TabButton label={t('analysis.tabs.deepDive')} icon={<LightBulbIcon className="w-5 h-5" />} isActive={activeTab === 'deepDive'} onClick={() => setActiveTab('deepDive')} activeColor="border-purple-500" />
          </nav>
        </div>
